	...
	export default {
		data() {
			return {
				...
				coach_full_img : 'https://p1.nanmuxuan.com/images/46/62a69fcb688d68f7.jpg',
				birthday : '1966-9-1',
				birthcity : 'New York',
		...
		onLoad(option) {
			...
			uni.request({
				...
				success: res => {
					this.coach_img = res.data.photo;
					this.coach_name = res.data.name;
					this.coach_full_img = res.data.model_3d;
					this.birthday = res.data.date_of_birth;
					this.birthcity = res.data.city_of_birth;
					this.university = res.data.university;
					this.senior_high_school = res.data.senior_high_school;
					this.career_number = res.data.career_number;
					this.career_team_number = res.data.career_team_number;
					this.coach_data = res.data.career;
					this.avg_data = {
						win_rate : res.data.career_win_rate,
						win : res.data.career_win,
						lose : res.data.career_loss
					}
				},
				fail: () => {},
				complete: () => {}
			...
			for(let i=0;i<this.coach_data;i++){
				categories.push(this.coach_data[i].season);
				series.data.push(this.coach_data[i].win_rate)
			}
